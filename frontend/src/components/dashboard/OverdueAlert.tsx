...
export function OverdueAlert({ equipment }: OverdueAlertProps) {
    ...
    return (
        <div className="section-panel bg-red-50/50 border-red-200 border-l-[3px] border-l-red-500 rounded-xl p-3 shadow-sm">
                ...
                <div className="flex items-center gap-4">
                    <div className="hidden sm:flex items-center gap-2">
                        {overdue.slice(0, 2).map((e) => (
                            <span
                                key={e.id}
                                className="inline-flex items-center gap-1 bg-white border border-red-100 px-2 py-0.5 rounded-full text-xs font-mono text-red-700"
                            >
                                {e.name}
